...
$( document ).ready( function(){
	...
	*/
	...
	var sideNavHide = '.side-nav-hide a, .one-page-nav .side-navigation .scroll-link';						// Side nav hide button.
	...
	*/
	...
	var menuContainer = '.side-navigation-wrapper nav, .overlay-navigation-wrapper nav';	// Containers
		...
		init: function(){
			...
			$( menuContainer ).find( subMenuParentLink ).each( function(){
				...
				$( this ).on( eventType, function( event ) {
					...
					if( $( this ).parent().hasClass( 'current' ) ){
						...
						if( $( this ).parents().eq( 2 ).is( 'nav' ) ) $( this ).parents().eq( 2 ).find( 'li' ).removeClass( 'current' );
			...
			function closeSubMenus( link ){
				if( $( link ).parents().eq( 2 ).is( 'nav' ) ){
					$( link ).closest( 'nav' ).find( subMenu ).css({ height: 0 }).removeClass( 'open' );
				}else{
	...
	*/
	...
	var components = {
		...
		collapsable: function(){
			...
			var targetContent;
			var targetContentHeight;

			// On event
			$( collapsable ).each( function(){
				var link = $( this );
				var includeMargin = link.is( '[data-include-margin]' ) ? true : false;
				if( link.hasClass( 'active' ) ) calcTargetHeight( link, includeMargin );

				// Link
				link.on( eventType, function( event ){
					event.preventDefault();
					var link = $( this );
					calcTargetHeight( $( this ), includeMargin );
			...
			function calcTargetHeight( link, includeMargin ){
				var collapsableTarget = link.attr( 'href' );
				$( collapsableTarget ).addClass( 'include-margin' );
				targetContent = $( collapsableTarget ).children();
				targetContentHeight = 0;
				targetContent.each( function(){
					targetContentHeight += $( this ).outerHeight( includeMargin );
				});
				$( collapsableTarget ).css({ height: targetContentHeight + 'px' });
			}

			// Toggle target
			function toggleTarget( link ){
				var collapsableTarget = link.attr( 'href' );
				$( collapsableTarget ).addClass( 'collapsable-target' );
				if( link.hasClass( 'active' ) ){
					link.removeClass( 'active' );
					$( collapsableTarget ).removeClass( 'active' );
					$( collapsableTarget ).css({ height: 0 + 'px' });
				}else{
					link.addClass( 'active' );
					$( collapsableTarget ).addClass( 'active' );
				}
			}

			// On window resize
			// set collapsable target height auto
			$( window ).on( 'resize', function(){
				$( collapsable ).each( function(){
					var collapsableTarget = $( this ).attr( 'href' );
					if( $( this ).hasClass( 'active' ) ) $( collapsableTarget ).css({ height: 'auto' });
				});
			});
		},
			...
			$( dropdown ).each( function(){
				$( this ).children( 'a, .button, button' ).each( function(){
					...
					ddButton.on( eventType, function( event ){
						event.preventDefault();
						$( dropdown ).children( 'a, .button, button' ).removeClass( 'active' );
						var dropdownList = $( this ).parent().children( '.dropdown-list' );
						if( $( this ).parent().hasClass( 'disabled' ) ) return false;
						if( $( this ).parent().hasClass( 'list-up' ) ){
							listHeight = $( dropdown ).children( 'a, .button, button' ).outerHeight() + calcListHeight( dropdownList ) + 8;
